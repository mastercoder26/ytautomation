...
import { ProcessExecutionError } from "../../src/media/process.js";
...
describe("restricted PDF worker", () => {
  it("extracts text in a network/child/write-disabled Node process", async () => {
    ...
    const extracted = await extractPdfText(path, [root]).catch((error: unknown) => {
      if (error instanceof ProcessExecutionError) throw new Error(error.stderr);
      throw error;
    });
